...
import numpy as np
from read import ShapeNetLoader
...
def conv2d_block(inputs, out_channels, kernel_size=2, stride=1, activation=tf.nn.relu, batch_norm=True, use_bias=False, layer_name='conv2d_layer_'):
    ...
    return activation(convolution, name='_'+activation.__name__)
...
def generate_samples(sess, path, imgs, epoch, threshold=0.9):
    ...
    for i in range(imgs.get_shape()[0]):
        np.save(path+'/out_'+epoch+'_'+(i+1)+'.obj', _output_volumes[i])

with tf.Session() as sess:
    sess.run(tf.global_variables_initializer())
    saver = tf.train.Saver()
    data = ShapeNetLoader()
    data.load_data('/Users/aiwabdn/Downloads/')
    ...
    for i in range(EPOCHS):
        ...
        print('L_enc : ',loss_e, '|| L_gen : ', loss_g, '|| L_dis : ', loss_d, ' || L_kl : ', loss_k)
    saver.save(sess, '/Users/aiwabdn/Downloads/output/gan.ckpt')
    generate_samples(sess, '/Users/aiwabdn/Downloads/output', img[:1], 10)
