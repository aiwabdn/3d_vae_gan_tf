...
with tf.variable_scope('conv1') as scope:
    ...
    c = tf.nn.conv2d(input_img, kernel, [1,1,1,1], padding='SAME')

    # apply relu on the sum of convolution output and biases
    conv1 = tf.nn.relu(c + b, name=scope.name)
...
with tf.variable_scope('fc') as scope:
    ...
    w1 = tf.get_variable('weights', [input_features,1024], initializer=tf.truncated_normal_initializer())
    ...
    c1 = tf.matmul(pool2, w1)
    conv3 = tf.nn.relu(c1 + b1)

    # apply dropout
    fc = tf.nn.dropout(conv3, dropout, name='relu_dropout')

with tf.variable_scope('softmax_linear') as scope:
    # this you should know. get logits without softmax
    # you need to create weights and biases
    w2 = tf.get_variable('weights', [1024,N_CLASSES], initializer=tf.truncated_normal_initializer())
    b2 = tf.get_variable('biases', [N_CLASSES], initializer=tf.random_normal_initializer())
